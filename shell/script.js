...
const Terminal = ({ theme, setTheme }) => {
  ...
  const handleClose = () => (window.location.href = "https://markwarrior.com");
  const handleMinMax = () => {
...
class Field extends React.Component {
  constructor(props) {
    ...
    this.recognizedCommands = [
      ...
      {
        command: "marco",
        purpose: "Shows a bunch of stuff",
      },
      {
        command: "time",
  ...
  handleInputExecution(cmd, params = [], flags = []) {
    if (cmd === "help") {
      ...
      return this.setState(
        ...
        () => window.open("https://markwarrior.com")
      );
    } else if (cmd === "theme") {
      ...
      return (window.location.href = "https://markwarrior.com");
    } else if (cmd === "time") {
      return this.setState((state) => ({
        fieldHistory: [
          ...state.fieldHistory,
      ...
      return this.setState((state) => ({
        fieldHistory: [
          ...
          {
            text: [
              "Email: hello@markwarrior.com",
              "Website: marwarrior.com",
              "LinkedIn: Not",
              "GitHub: @huntinghawk1415",
              "CodePen: @HuntingHawk",
            ],
      ...
      return this.setState((state) => ({
        fieldHistory: [
          ...state.fieldHistory,
          {
            text: ["Nome de alguem que gosta muito de levar no rabinho :)"],
            hasBuffer: true,
          },
        ],
      }));
    ...
    return React.createElement(
      ...
      fieldHistory.map(({ text, isCommand, isError, hasBuffer }) => {
        if (Array.isArray(text)) {
          return React.createElement(MultiText, {
            input: text,
            isError: isError,
            hasBuffer: hasBuffer,
          });
...
const Text = ({ input, isCommand, isError, hasBuffer }) =>
  React.createElement(
    ...
    React.createElement(
      ...
      null,
      isCommand &&
        React.createElement("div", { id: "query" }, "C:\\Users\\Guest\\>"),
      React.createElement(
        "span",
